...
| FEAD (kayış-kasnak) | `fead-analysis` | `fead-analysis` | `js/cp-fead.js` | **YOK — kasnakların kanvasta KUTUSU BİLE yok** (`noCanvasBox`). Sıra, koordinat ve çap **Kayış Tablosu**'ndan; detay panele tablodaki ada tıklanarak gidilir. Şema kartı İKİYE bölündü: `fead-layout` donuk geometri, `fead-run` çalışma noktası; **ikisi de çoğaltılabilir ve her kart kendi KATMANLARINI seçer** (`node.data.kat`) |
...
- **KARTIN EN KÜÇÜK ÖLÇÜSÜNÜ TİPİ SÖYLER** (`componentDefs.minWidth/minHeight`
  ...
  kart için sessiz kayıp üretir (yapışkan başlık + Σ gövdeyi tamamen örter,
  sütunlar işaretsiz kayar). Beyan etmeyen tip eski tabanda kalır; taban hem
