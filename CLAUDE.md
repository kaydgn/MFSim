...
| FEAD (kayış-kasnak) | `fead-analysis` | `fead-analysis` | `js/cp-fead.js` | **YOK — kasnakların kanvasta KUTUSU BİLE yok** (`noCanvasBox`). Sıra, koordinat ve çap **Kayış Tablosu**'ndan — o da bir ızgara değil **KART LİSTESİ** (satır = kasnak; kim · gir · coz bölgeleri); detay panele satırdaki ad DÜĞMESİNE tıklanarak gidilir. Kanvas kartının **TEK tipi var** (`fead-layout`); geometri ↔ işletme ayrımı bir **ÖN AYAR** (`node.data.katOn`), tip değil; kartta **yatay bant YOK** — denetimler çizimin üstünde yüzer. Kart **çoğaltılabilir** ve her kart kendi KATMANLARINI seçer (`node.data.kat`) |
...
- **KART İÇİNDEKİ KAYDIRILABİLİR YÜZEY TEKERLEĞİ ÖNCE ALIR**
  ...
  **ve** içerik sığmıyor; yalnız birincisi taşması olmayan her kapta tekerleği
  yutar. Kapı: `fead-tablo.spec.js` → *"TEKERLEK"*.
...
| `tests/unit/fead-table.test.js` | `js/cp-fead.js` Kayış Tablosu + `js/fead-model.js` sıra | **Kasnakların veri giriş yüzeyi**: sütun kimlikleri kullanıcının hesap sayfasına karşı ölçülü (efektif çap = OD+2·hb / OD+2·hr, Σişaretli sarım 360°, L = Σspan + Σ(sarım·r)); türetilen sütunlar ÇEKİRDEKTEN (tablo kendi geometrisini hesaplamaz); gergi satırı `cenX/cenY` okur; **girdi sütunları geometri çözülemese de dolu** (tablo bir rapor değil, giriş yüzeyi); virgüllü ondalık kabul (`63,5` → 63.5, `parseFloat` sessizce 63 verirdi); satır taşıma sırayı değiştirir, **sürücü satırı kilitli**; kart kanvasta olduğu için her hücre `mousedown` yutar; **tazeleme TEK KAPIDAN** (`veFeadRefreshCards` — şema ve tablo birlikte, kaynak kapısıyla); **Dönüş Yönü defterdeki gibi bir GİRDİ** (iki durumlu segment — iki seçenek de tek bakışta sığıyor, tarayıcının oku listedeki en göze batan parçaydı) ama `contact` alanını yazar — ikinci bir yön alanı doğmaz, süpürme işareti okunamıyorsa yazma reddedilir, ve **defterin sessiz tutarsızlığı kurulamıyor**: yönü değiştirmek efektif çapı DA değiştiriyor (2·hr → 2·hb); Kayış Uzunluğu KÜNYEDE (`rowspan`lı hücre beş satır boyu bir dikdörtgenin ortasında tek sayı taşıyordu — boy satıra değil ÇEVRİME ait); **kutular kalkınca ekleme/silmenin TEK yolu tablo** — ekleyici tip listesi `componentDefs`ten türer, eklenen kasnak sıranın SONUNA düşer, satır ✕'i diziden çıkarıp sırayı 1..N-1'e kapatır ve `saveState`i mutasyondan ÖNCE çağırır ; **görünüm CSS'te, satır içinde DEĞİL** — satır içi CSS `:hover`/`:focus`/`:nth-child` ifade edemediği için tablo donuktu; kapı ÇİFT ve ayrı ayrı hiçbir şey ifade etmezler: kart HTML'i satır içi renk/çerçeve/yazı YAZMIYOR (yalnız `--fead-krt-*` bölge genişlikleri — VERİ; `coz` bölgesininki YAZILMAZ, o `1fr`) + o rengi veren durum kuralları `css/styles.css`'te VAR ve hepsi `--accent-tint-*`/`--focus-ring` jetonundan; **seçili satır** paneli açık olan kasnağı gösterir ve seçim değişince kart KURULMADAN sınıf eşitlenir (`veFeadMarkSelectedRow`, cp-core'un iki merkezinden çağrılır — ölçülen hata: işaret hiç tazelenmiyor, tabloda paneli açık OLMAYAN satır işaretli kalıyordu); başlıkta ad ile birim ayrı satırda; Σ toplam KÜNYEDE ve gösterilen alanların toplamı (yeni büyüklük türetmez); boş durum artık paleti değil tablonun kendi ekleyicisini gösteriyor (palet kutular kalkınca SESSİZ); kart ölçüsü 870×340 → **770×360** ve eski ölçü yükseliyor — genişlik düştü (`rowspan`lı sütun künyeye geçti) ve bölge toplamıyla TAM oturuyor (782'de sağda 12 px ölü şerit vardı); **taban artık kart genişliğinin ALTINDA** (564 < 770) çünkü bölgeler `minmax` ile daralıyor ve hiçbir alan kaybolmuyor; **IZGARA DEĞİL KART LİSTESİ** (`.ve-fead-krt`, üç bölge: kim · gir · coz) ve **girdi/çözüm ayrımı BÖLGENİN KENDİSİ** — her sütunun `yer` alanı `coz` bayrağıyla tutarlı, çözüm bölgesinde yazılabilir hiçbir eleman yok, girdi bölgesinde okuma yok (kural, liste değil); alan etiketleri KISA, defterin tam adı `title`da; türetilen sayılar TEK basamak (üçüncüsü hassasiyet değildi); **çevrim denetimi ÜST KÜNYEDE** okunur Türkçeyle (alt şerit ve `L_pitch`/`L_eff` kısaltmaları kalktı — ✗ dalı `od=5000` ile ve "çözüldü ama Σ≠360" hâliyle ayrı ayrı kapılı); ekleyici tablonun ALTINDA (eklenen kasnak oraya düşüyor); sürücü satırın SINIFINDA da işaretli, seçim sınıfıyla birlikte durabiliyor; **ad hücresi bir BAĞLANTI** — kasnak panelinin tek yolu o hücre ve okunur bir metin olarak dururken varlığı ancak deneyerek keşfediliyordu: her zaman görünen "pencere açılır" simgesi (yazı karakteri değil ÇİZİM — eksik glif afordansı yok ederdi) + gölgeyi kırpmayan hücre; **EN KÜÇÜK ÖLÇÜ** tipten okunuyor ve sütun toplamından geri kalmıyor (50×50 tabanında yapışkan başlık + Σ gövdeyi tamamen örtüyordu), taban AÇILIŞTA da uygulanıyor — kuraldan önce küçültülüp kaydedilmiş kart yükseliyor; **eklenen satır görüş alanına alınıyor** (`block:'nearest'` — görünen satır listeyi zıplatmaz) ve kaydırma tazelemeden SONRA çağrılıyor (satır o çağrıyla doğuyor) |
| `tests/e2e/fead-tablo.spec.js` + `fead-sihirbaz-tablo.spec.js` | Kayış Tablosu (gerçek tarayıcı) | **Kart kanvasta**: monte olur, kasnaklar arası tel 0, gerçek `63,5` yazımı modeli VE çözümü değiştirir (`type="number"` olsaydı tarayıcı virgülü yutar, model hiç değişmezdi), satır oku sırayı değiştirir, sürücü satırı kilitli, yön segmentinin düğmesi `contact`i çevirip efektif çapı +0,2 mm oynatır, kayış boyu künyede tek. **Sihirbazın 'Modeli Kur'u**: adımlar gezilir, gerçek düğmeye tıklanır → 6 kasnak + tablo + şema + İŞLETME kartı + kayış + çözücü + rapor + sihirbaz = 13 düğüm, 0 bağlantı, indis 1..N, çözüm önizlemeyle birebir, **uyarı toast'ı yok** (tablo yeniden kullanılır, ikincisi kurulmaz) ve **araç çubuğu sayacı bayat değil** (örnek düğümü splice edildiği için kurucu `updateNodeCount`'u kendisi çağırır). **AÇILIŞ SİHİRBAZLA KARŞILIYOR** (boş FEAD topolojisi → modal açık; kapatınca iç topoloji ayakta) ve "Başlangıç ve Örnekler" bileşeni açılış listesinde YOK; **KUTUSUZLUK kapısı ikisinde de**: kasnakların DOM elemanı 0, kanvastaki her `.ve-node` bir araç düğümü; satır ✕'i kasnağı siler, "＋ Kasnak ekle" sıranın sonuna ekler ve eklenenin de kutusu olmaz ; **DURUM GERİ BİLDİRİMİ ölçülüyor** (jsdom `:hover`ı da `:focus`u da hiç hesaplamaz, kapı Node'a taşınamaz): fare satır zeminini değiştiriyor, **zebra KALKTI** (komşu satırlar aynı zemin) ve yerine çözüm BÖLGESİNİN gömülü zemini geldi; silme dinlenmede görünmez ama satıra gelince VE odaklanınca belirir, hücreye odaklanınca halka beliriyor, adı tıklanan kasnağın satırı işaretleniyor ve seçim boşalınca işaret kalkıyor, tablo kabına yatay kaydırmasız sığıyor; sürücü satırının okları artık `<span>` değil `disabled` düğme; **TEKERLEK listenin**: taşma yokken kanvas yakınlaşıyor, taşma varken tablo kayıyor ve kanvas OYNAMIYOR, künye şeridinde yine kanvas (kanvasın kayıtsız `preventDefault()`u yüzünden kart içindeki hiçbir liste kaydırılamıyordu); **gerçek tutamak sürüklemesi TABANDA duruyor** ve tabandaki kart hâlâ en az iki satır gösteriyor, hiçbir sütun kaymıyor; **eklenen kasnağın satırı her eklemede tam görünür**; **ad düğmesinin GÖLGESİ ölçülüyor** (dinlenmede yok → fare üstündeyken var, hücre `overflow:visible` olmasa gölge sessizce hiç çizilmezdi) ve **CTRL+Z tabloyu SİLMİYOR**: onikilik örnek tek adımda geri alınıyor, daha fazla basmak açılış yüzeyinde duruyor, ileri al tek adımda geri getiriyor |
...
| `tests/unit/loader-splash.test.js` | `js/loader.js` + `index.html` açılış ekranı | **Açılış ekranı**: splash gövdesi ↔ `ELS` kimlik sözleşmesi (bir yeniden adlandırma çubuğu sessizce durdururdu), `data-mfsim-stage` işaretlerinden aşama öbeklerinin kurulması (işaretsiz script bir öncekine yazılır; ad `&` içerebilir), atlanan modülün GÖRÜNÜR olması + yüklemenin devam etmesi, sürüm künyesi (modüler kopyada boş kalır), ipucu döngüsünün kapanışta DURMASI; **SAHNE**: kare listesinin defer OLMAMASI ve yükleyiciden ÖNCE gelmesi (defer setine düşerse açılış fotoğrafsız açılır), karenin seçilip boyanması + gömülü data URI ↔ dosya yolu, kare yoksa kâğıt zeminde açılıp yüklemenin yine bitmesi, kartın karşılama kartıyla BİREBİR geometrisi (left/genişlik/yarıçap/cam/gölge — 1 px fark "kart yerinden oynadı" demek) ve kapanışta kaymaması, **temanın ilk karede** uygulanması: belgenin varsayılanı js/theme.js'inkiyle aynı, kayıtlı tema hiçbir şey çizilmeden konuyor, bozuk kayıt yazılmıyor ve geçerlilik listesi loader'da KOPYALANMIYOR; **KADEME**: kademe sayısı tek kaynak (CSS jetonu `--mfsim-kademe`, loader onu OKUR — iki yerde yazılsa dolgu çentikle hizalanmazdı), ilerlemenin kademeye yuvarlanması (1/24'te çubuk hâlâ %0), genişliğin YUVARLANMIŞ yüzdeden değil q'dan gelmesi (8 yazılsaydı dolgu çentiğin 0,3 punto gerisinde kalırdı), yükleme bitince %100'e oturması, dişlinin kademe başına bir çentik dönüp %100'de TAM TUR tamamlaması (devir teslimde dik durur), modül adının da kademeyle değişmesi, süsleme karelerinin (dönen dişli + parıltı) KALDIRILDIĞI, biten öbeğin işaretinin bir kez vurulup sınıfın kalkması |
