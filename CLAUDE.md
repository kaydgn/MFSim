...
| `tests/unit/port-geometry.test.js` | `js/components.js` port geometrisi + `js/connections.js` | Bağlantı ucu ile port dairesi aynı noktada — dört kenar, çok port, aynalama; **gidiş yönü oku** (Bézier t=0.5, 46 px eşiği, ters yön); **`veSyncPortDom`** — kenar sonradan değişince (bağlantı/sürükleme) dairenin teli takip etmesi, elle taşınan portun ezilmemesi, kenar değişmiyorsa DOM'a hiç yazılmaması; **kayış telinin gidiş oku telin YÖNÜNDE** — tel kayışın gidişidir, köprü çekirdeğe ters listeyi verir (ilk turda tel tablo sırasındaydı ve ok tersine çizilmişti); ters kablolamada ok döner; kayış olmayan telde ok yok |
