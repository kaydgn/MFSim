...
function veSolverValidate() {
  ...
  if(allOk) {
    ...
    html += '<div style="margin-top:12px;padding:10px;background:rgba(76,175,80,0.1);border-radius:2px;border:1px solid rgba(76,175,80,0.3);text-align:center;font-size:0.8rem;color:var(--accent-success);font-weight:600;">✅ ' + modeText + ' - hesaplamaya hazır</div>';
    document.getElementById('ve-solver-run-btn').disabled = false;
    document.getElementById('ve-solver-run-btn').style.opacity = '1';
  } else {
    html += '<div style="margin-top:12px;padding:10px;background:rgba(244,67,54,0.1);border-radius:2px;border:1px solid rgba(244,67,54,0.3);text-align:center;font-size:0.8rem;color:var(--accent-danger);font-weight:600;">❌ Eksikler var - hesaplama yapılamaz</div>';
  ...
  setTimeout(function() {
    ...
    setTimeout(function() {
      try {
        ...
        rhtml += '<div style="background:var(--bg-tertiary); padding:10px; border-radius:2px; border:1px solid var(--border-color); text-align:center;"><div style="font-size:0.65rem; color:var(--text-muted);">Toplam Süre</div><div style="font-size:1rem; font-weight:700; color:var(--accent-primary);">' + totalTime.toFixed(1) + ' s</div></div>';
        rhtml += '<div style="background:var(--bg-tertiary); padding:10px; border-radius:2px; border:1px solid var(--border-color); text-align:center;"><div style="font-size:0.65rem; color:var(--text-muted);">Çıktı Noktası</div><div style="font-size:1rem; font-weight:700; color:var(--accent-success);">' + simResult.time.length + '</div></div>';
        
        if(simResult.mode === 'partial') {
          var maxRpm = Math.max.apply(null, simResult.rpm);
          var finalRpm = simResult.rpm[simResult.rpm.length - 1];
          rhtml += '<div style="background:var(--bg-tertiary); padding:10px; border-radius:2px; border:1px solid var(--border-color); text-align:center;"><div style="font-size:0.65rem; color:var(--text-muted);">Maks Devir</div><div style="font-size:1rem; font-weight:700; color:#ef4444;">' + maxRpm.toFixed(0) + ' rpm</div></div>';
          rhtml += '<div style="background:var(--bg-tertiary); padding:10px; border-radius:2px; border:1px solid var(--border-color); text-align:center;"><div style="font-size:0.65rem; color:var(--text-muted);">Son Devir</div><div style="font-size:1rem; font-weight:700; color:#f59e0b;">' + finalRpm.toFixed(0) + ' rpm</div></div>';
        } else {
          var maxV = Math.max.apply(null, simResult.speed);
          var finalV = simResult.speed[simResult.speed.length - 1];
          rhtml += '<div style="background:var(--bg-tertiary); padding:10px; border-radius:2px; border:1px solid var(--border-color); text-align:center;"><div style="font-size:0.65rem; color:var(--text-muted);">Maks Hız</div><div style="font-size:1rem; font-weight:700; color:#ef4444;">' + maxV.toFixed(2) + ' km/h</div></div>';
          rhtml += '<div style="background:var(--bg-tertiary); padding:10px; border-radius:2px; border:1px solid var(--border-color); text-align:center;"><div style="font-size:0.65rem; color:var(--text-muted);">Son Hız</div><div style="font-size:1rem; font-weight:700; color:#f59e0b;">' + finalV.toFixed(2) + ' km/h</div></div>';
        }
        ...
        if(ss.method) {
          rhtml += '<div style="background:var(--bg-secondary); border:1px solid var(--border-color); border-radius:2px; padding:10px; margin-bottom:10px;">';
