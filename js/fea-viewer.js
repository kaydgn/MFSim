...
function veFEAApplyPrimitive(nodeId, type, params) {
  ...
  // KRİTİK: showNodeProperties panel'i yeniden render eder, canvas DOM
  // elementi DEĞİŞİR (innerHTML = ...). Eski WebGL context detached canvas'a
  // bağlı kalır; Chrome'un ~16 concurrent WebGL context limiti var. Bu yüzden
  // showNodeProperties çağrısından ÖNCE eski viewer'ı dispose ediyoruz —
  // WEBGL_lose_context ile slot serbest. Sonra cp-core setTimeout(100) ile
  // veFEAInitGeometryViewerForNode yeni canvas'ta taze context oluşturur.
  // Aksi halde "Cannot read properties of null (reading 'precision')" hatası
  // (context limit aşımı, getShaderPrecisionFormat null döndürür).
  if (veFEAViewerRegistry[nodeId]) {
    try { veFEAViewerRegistry[nodeId].dispose(); } catch(e) {}
    delete veFEAViewerRegistry[nodeId];
  }
