...
function veRefreshApp() {
  ...
  _veCheckDeploy(function(info) {
    ...
    // Deploy başarılıysa "Güncelleme Mevcut" popup'ı + "Şimdi Güncelle" butonu
  ...
  if(wasRefreshed) {
    ...
    setTimeout(function() {
      _veCheckDeploy(function(info) {
        if(info) {
          var dot = document.getElementById('ve-deploy-dot');
          _veShowPopup(dot, info, '✅ Program Güncellendi', false);
        }
      });
    }, 1500);
  }
  // Otomatik sorgu yok — sadece 🔄 butonuyla veya yeşil noktayla kontrol edilir
});
