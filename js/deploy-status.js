...
var DEPLOY_POLL_INTERVAL = 120000; // 2 dakika (GitHub API limiti: 60 istek/saat)
var _deployTimerId = null;
var _deployKnownRunId = null; // Sayfa yüklendiğindeki son deploy ID
var _deployInitDone = false;
var _deployRateLimited = false; // API rate limit durumu
  ...
  fetch('https://api.github.com/repos/' + DEPLOY_REPO + '/actions/runs?per_page=1&branch=main')
    .then(function(r) {
      ...
      if(r.status === 403 || r.status === 429) {
        _deployRateLimited = true;
        var retryAfter = r.headers.get('Retry-After');
        var waitMs = retryAfter ? parseInt(retryAfter) * 1000 : 300000; // 5 dk varsayılan
        _veSetDeployDot(dot, 'ratelimit', null);
        _veScheduleNextCheck(waitMs);
      ...
      _deployRateLimited = false;
      return r.json();
    })
    .then(function(data) {
      if(!data) return; // Hata durumunda zaten işlendi
...
// ═══ ZAMANLAYICI ═══
function _veScheduleNextCheck(overrideMs) {
  if(_deployTimerId) clearTimeout(_deployTimerId);
  var ms = overrideMs || DEPLOY_POLL_INTERVAL;
  _deployTimerId = setTimeout(function() { veCheckDeployStatus(); }, ms);
}
...
function _veSetDeployDot(dot, state, info) {
  ...
  if(!info) {
    if(state === 'offline') dot.title = 'Bağlantı yok';
    else if(state === 'ratelimit') dot.title = 'API limiti aşıldı — birkaç dakika sonra tekrar denenecek';
    else dot.title = 'Durum bilinmiyor';
    return;
  }
...
function _veApplyUpdate() {
  ...
  if('caches' in window) {
    caches.keys().then(function(names) {
      ...
      // SW varsa unregister et ki yeni versiyonu alsın
