...
function veGenerateTopologySummary() {
  ...
  // Aktif modül adı
  var modNames = {'engine-brake':'M. Freni Performans','full-throttle':'Tam Gaz Hızlanma','performance':'Araç Performans','fuel':'Yakıt Tüketimi'};
  var modLabel = modNames[veActiveModule] || veActiveModule || '—';

  lines.push(sep);
  lines.push('  TOPOLOJİ ÖZETİ — ' + (veProjectName || 'Proje'));
  lines.push(sep);
  lines.push('  Tarih           : ' + new Date().toLocaleString('tr-TR'));
  lines.push('  Aktif Modül     : ' + modLabel);
  ...
  nodes.forEach(function(node, idx) {
    ...
    if(node.type === 'engine' || node.type === 'engine-brake') {
      ...
      if(d.fitMethod) lines.push('│  Eğri Uydurma      : ' + (d.fitMethod === 'polynomial' ? 'Polinom (derece ' + (d.polyDegree||5) + ')' : 'Lineer'));
      // Motor Specs
      var sp = d.motorSpecs || {};
      ...
      if(d.accessories && d.accessories.length > 0) {
        var totalLoss = d.accessories.reduce(function(s,a) { return s + (a.powerLoss || a.userLoss || a.standardLoss || 0); }, 0);
        if(totalLoss > 0) {
          lines.push('│');
          lines.push('│  Aksesuar Kayıpları:');
          d.accessories.forEach(function(acc) {
            var loss = acc.powerLoss || acc.userLoss || acc.standardLoss || 0;
            if(loss > 0) {
    ...
    if(node.type === 'gearbox') {
      lines.push('│  Preset             : ' + v(d.selectedGearbox || d.ftGBPreset || d.gbName));
      lines.push('│  Verim              : %' + v(d.efficiency, 97));
      if(d.selectedGear || d.selectedGearRatio) {
        lines.push('│  Seçili Vites       : ' + v(d.selectedGear, '—', '. vites'));
        lines.push('│  Seçili Vites Oranı : ' + v(d.selectedGearRatio));
      }
      if(d.forwardGears) lines.push('│  İleri Vites Sayısı : ' + d.forwardGears);
      if(d.shiftProfile) lines.push('│  Shift Profili      : ' + d.shiftProfile);
      if(d.lockupOffset !== undefined && d.lockupOffset !== null) lines.push('│  Lockup Offset      : ' + d.lockupOffset);
      // Vites tablosu — MF modu
      ...
      // Vites tablosu — FT modu
      if(d.ftGearData && d.ftGearData.length > 0 && !(d.gearData && d.gearData.length > 0)) {
        ...
        d.ftGearData.forEach(function(g) {
          var nStr = String(g.name||g.gear||'?').padEnd(10);
    ...
    if(node.type === 'torque-converter') {
      if(d.tcPresetKey) lines.push('│  Preset             : ' + d.tcPresetKey);
    ...
    if(node.type === 'transfer') {
      ...
      if(d.selectedMode) lines.push('│  Seçili Mod         : ' + d.selectedMode);
      if(d.selectedRatio) lines.push('│  Seçili Oran        : ' + d.selectedRatio);
      // Kademe tablosu — MF modu
      if(d.transferData && d.transferData.length > 0) {
        ...
        d.transferData.forEach(function(t) {
          ...
          lines.push('│  │  ' + mode + ' → Oran: ' + ratio + sel);
        });
        lines.push('│  └──────────────────────────────────────────────────┘');
      }
      // Kademe tablosu — FT modu
      if(d.ftTrGears && d.ftTrGears.length > 0 && !(d.transferData && d.transferData.length > 0)) {
        lines.push('│');
        lines.push('│  ┌──── Kademe Tablosu ──────────────────────────────┐');
        d.ftTrGears.forEach(function(t) {
          var kademe = t.kademe || '?';
          var ratio = t.ratio || t.oran || '—';
          var effStr = t.eff || t.verim ? ', Verim: %' + (t.eff || t.verim) : '';
          lines.push('│  │  ' + kademe + ' → Oran: ' + ratio + effStr);
        });
        lines.push('│  └──────────────────────────────────────────────────┘');
      }
    }
    ...
    if(node.type === 'propshaft') {
      ...
      lines.push('│  Verim              : %' + v(d.psEff, 98.60));
      lines.push('│  Atalet             : ' + v(d.psInertia, 0.5, ' kg·m²'));
    }
    ...
    if(node.type === 'wheel') {
      ...
      else if(nodes.filter(function(nn){return nn.type==='wheel';}).length > 1) lines.push('│  SLAVE (master tarafından kontrol ediliyor)');
      // MF modu alanları
      if(d.tirePreset)          lines.push('│  Preset             : ' + d.tirePreset);
      if(d.wheelRadius)         lines.push('│  Tekerlek Yarıçapı  : ' + d.wheelRadius + ' m');
      if(d.rollingResistance !== undefined && d.rollingResistance !== null && d.rollingResistance !== '')
        lines.push('│  Crr (yuvarlanma)   : ' + d.rollingResistance);
      if(d.rotatingMass)        lines.push('│  δ (döner kütle)    : ' + d.rotatingMass);
      // FT modu alanları
      if(d.ftTirePreset)        lines.push('│  Preset             : ' + d.ftTirePreset);
      if(d.ftTireName)          lines.push('│  Lastik Adı         : ' + d.ftTireName);
      if(d.ftTireRadius)        lines.push('│  Lastik Yarıçapı    : ' + d.ftTireRadius + ' m');
      if(d.ftTireInertia)       lines.push('│  Lastik Ataleti     : ' + d.ftTireInertia + ' kg·m²');
      if(d.ftCrr !== undefined && d.ftCrr !== null && d.ftCrr !== '')
        lines.push('│  Crr (yuvarlanma)   : ' + d.ftCrr);
      if(d.ftSurfaceFactor !== undefined && d.ftSurfaceFactor !== null && d.ftSurfaceFactor !== 1.00)
        lines.push('│  Yüzey Katsayısı    : ' + d.ftSurfaceFactor);
      // Coast-down Crr
    ...
    // ── SOLVER ──
    if(node.type === 'solver') {
      var tmMap = {'stop': 'Durma Analizi', 'manual': 'Manuel'};
      lines.push('│  Zaman Modu         : ' + (tmMap[d.timeMode] || d.timeMode || 'Manuel'));
      lines.push('│  Süre               : ' + v(d.duration, 60, ' s'));
      lines.push('│  Maks. Sim. Süresi  : ' + v(d.maxSimTime, 300, ' s'));
      ...
      if(d.method === 'rk45') {
        lines.push('│  Çıktı Noktası      : ' + v(d.resolution, 200));
      ...
      // FT solver parametreleri
      if(d.ftDt) lines.push('│  FT Zaman Adımı     : ' + d.ftDt + ' s');
      if(d.ftAtol || d.ftRtol) lines.push('│  FT ATol / RTol     : ' + v(d.ftAtol, 1e-6) + ' / ' + v(d.ftRtol, 1e-4));
      lines.push('│  Durma Eşiği        : ' + v(d.stopSpeed, 2, ' km/h'));
    }
    ...
    if(node.type === 'coast-down') {
      lines.push('│  Yöntem             : ' + (d.cdMethod === 'test' ? 'Test Verisi' : 'Manuel Crr'));
      if(d.cdMethod === 'test') {
        lines.push('│  v1 (başlangıç)     : ' + v(d.v1, '—', ' km/h'));
        lines.push('│  v2 (bitiş)         : ' + v(d.v2, '—', ' km/h'));
        if(d.t1 !== undefined) lines.push('│  t1                  : ' + d.t1 + ' s');
        if(d.t2 !== undefined) lines.push('│  t2                  : ' + d.t2 + ' s');
      }
    ...
    if(node.type === 'ec-matching') {
      ...
      if(d.turbineRating) lines.push('│  Türbin Ratingi     : ' + d.turbineRating);
    }
  ...
  // ── GÜÇ AKTARMA ZİNCİRİ DİYAGRAMI ──
  lines.push(sep);
  lines.push('  GÜÇ AKTARMA ZİNCİRİ');
  lines.push(sep);
  ...
  function buildChain() {
    ...
    function walk(nodeId) {
      ...
      if(!node) return;
      if(skipTypes.indexOf(node.type) >= 0) return;
      chain.push(node);

      // Bu node'dan çıkan bağlantıları bul
      connections.forEach(function(c) {
        if(c.from === nodeId) walk(c.to);
      });
    }
  ...
  if(chain.length > 0) {
    ...
    function chainLabel(node) {
      ...
      if(node.type === 'engine' || node.type === 'engine-brake') {
        ...
        var gr = d.selectedGear || '?';
        var ratio = d.selectedGearRatio || '?';
        info = gr + '. vites, i=' + ratio;
      } else if(node.type === 'transfer') {
        info = (d.selectedMode||'?') + ', i=' + (d.selectedRatio||'?');
      } else if(node.type === 'propshaft') {
        info = 'η=' + (d.psEff||98.60) + '%';
      } else if(node.type === 'differential') {
        info = 'i=' + (d.diffRatio||'?');
      } else if(node.type === 'wheel') {
        var wr = d.wheelRadius || d.ftTireRadius || '?';
        info = 'r=' + wr + 'm';
      }
    ...
    // Tek satır akış
    var flowParts = chain.map(function(n) { return chainLabel(n); });
    var flowLine = flowParts.join(' → ');

    // Satır çok uzunsa parçala
    if(flowLine.length <= 65) {
      lines.push('  ' + flowLine);
    } else {
      lines.push('  ' + flowParts[0]);
      for(var fi = 1; fi < flowParts.length; fi++) {
        lines.push('    → ' + flowParts[fi]);
      }
    }
  } else {
    lines.push('  (Motor bileşeni bulunamadı veya bağlantı yok)');
  }

  // Araç ve yol bilgisini de ekle
  var vehN = nodes.find(function(n){return n.type==='vehicle';});
  var roadN = nodes.find(function(n){return n.type==='road';});
  if(vehN || roadN) {
    lines.push('');
    if(vehN) {
      var _vd = vehN.data||{};
      var _mass = _vd.mass || _vd.ftGVW || '—';
      var _cd = _vd.cd || _vd.ftCd || 0.65;
      var _v0 = _vd.initialSpeed || 0;
      lines.push('  Araç   : ' + _mass + ' kg, V₀=' + _v0 + ' km/h, Cd=' + _cd);
    }
    if(roadN) {
      var rd = roadN.data||{};
      lines.push('  Yol    : %' + (rd.grade!==undefined?rd.grade:0) + ' eğim, ' + (rd.altitude||0) + ' m, ' + (rd.temperature!==undefined?rd.temperature:15) + ' °C');
    }
  }
  lines.push('');
  ...
  lines.push(sep);
  lines.push('  AKTARMA ORGANLARI');
  lines.push(sep);

  // Verim
  lines.push('  Verim:');
  lines.push('    Şanzıman           : %' + gbE + (gbN ? '' : ' (varsayılan)'));
  lines.push('    Transfer           : %' + trE + (trN ? '' : ' (varsayılan)'));
  var psTotalEff = 1.0;
  psNodes.forEach(function(psn, pi) {
    var psE = psn.data && psn.data.psEff !== undefined ? parseFloat(psn.data.psEff) : 98.60;
    psTotalEff *= psE / 100;
    var label = psNodes.length > 1 ? 'Propshaft #' + (pi+1) : 'Propshaft';
    lines.push('    ' + label.padEnd(19) + ': %' + psE);
  });
  lines.push('    Diferansiyel       : %' + dfE + (dfN ? '' : ' (varsayılan)'));
  var toplamV = (gbE/100) * (trE/100) * psTotalEff * (dfE/100) * 100;
  lines.push('    ' + thin.substring(4));
  ...
  lines.push('  Aktarma Oranları:');
  lines.push('    Şanzıman           : ' + iG.toFixed(3));
  lines.push('    Transfer           : ' + iT.toFixed(3));
  lines.push('    Diferansiyel       : ' + iD.toFixed(3));
  if(tcN) lines.push('    TC Çarpanı         : ' + iC.toFixed(3) + (tcN.data && tcN.data.isLocked !== false ? ' (kilitli)' : ''));
  lines.push('    ' + thin.substring(4));
  ...
  if(!hasEngine)  warnings.push('Motor bileşeni eksik');
  if(!hasGearbox) warnings.push('Şanzıman bileşeni eksik');
  if(!hasDiff)    warnings.push('Diferansiyel bileşeni eksik');
  if(!hasWheel)   warnings.push('Tekerlek bileşeni eksik');
  if(!hasVehicle) warnings.push('Araç bileşeni eksik');
  if(!hasSolver)  warnings.push('Solver bileşeni eksik');
  if(!hasRoad)    warnings.push('Yol bileşeni eksik');

  // Bağlantı kontrolü — her bileşenin portları bağlı mı?
  nodes.forEach(function(n) {
    if(skipTypes.indexOf(n.type) >= 0 && n.type !== 'vehicle') return;
    var def = componentDefs[n.type] || {};
    var name = n.customName || def.name || n.type;
    // Giriş portu kontrolü
    if(def.inputs && def.inputs > 0) {
      var hasInput = connections.some(function(c) { return c.to === n.id; });
      if(!hasInput) warnings.push(name + ' — giriş portu bağlı değil');
    }
    // Çıkış portu kontrolü (tekerlek ve terminatör hariç)
    if(def.outputs && def.outputs > 0 && n.type !== 'wheel' && n.type !== 'terminator') {
      var hasOutput = connections.some(function(c) { return c.from === n.id; });
      if(!hasOutput) warnings.push(name + ' — çıkış portu bağlı değil');
    }
  });
  ...
  if(hasEngine) {
    ...
    if(!ed.governedRpm && !(ed.motorSpecs && ed.motorSpecs.governedSpeed)) warnings.push('Motor governed RPM tanımsız');
  }
  if(hasGearbox) {
    var gN = nodes.find(function(n){return n.type==='gearbox';});
    var gd = gN ? gN.data||{} : {};
    if(!gd.gearData && !gd.ftGearData) warnings.push('Şanzıman vites tablosu boş');
    if(!gd.selectedGear && !gd.selectedGearRatio) warnings.push('Şanzıman vites seçilmemiş');
  }
  if(hasWheel) {
    var wN = nodes.find(function(n){return n.type==='wheel' && n.isMasterWheel;}) || nodes.find(function(n){return n.type==='wheel';});
    var _wd = wN ? wN.data||{} : {};
    var crr = _wd.rollingResistance || _wd.ftCrr;
    if(crr !== undefined && crr !== null && crr !== '') {
      var crrN = parseFloat(crr);
      if(!isNaN(crrN)) {
        if(crrN < 0.002) warnings.push('Tekerlek Crr (' + crrN + ') alışılmadık düşük — doğrulayın');
        if(crrN > 0.02) warnings.push('Tekerlek Crr (' + crrN + ') alışılmadık yüksek — doğrulayın');
      }
    }
  }
  ...
  if(trNode) {
    var td = trNode.data||{};
    if(td.transferData && td.transferData.length > 0 && !td.selectedMode) {
      warnings.push('Transfer kademe tablosu var ama mod seçilmemiş');
    }
  }

  // Simülasyon hazırlık kontrolleri
  if(hasEngine && hasGearbox && hasDiff && hasWheel && hasVehicle && hasSolver && hasRoad) {
    checks.push('Tüm zorunlu bileşenler mevcut');
  }
  if(chain.length >= 4) {
    checks.push('Güç aktarma zinciri bağlı (' + chain.length + ' bileşen)');
  }
  ...
  if(checks.length === 0 && warnings.length === 0) {
    lines.push('  ✓ Kontrol edilecek öğe yok');
  }
  lines.push('');
  lines.push(sep);
...
function veShowTopologySummary() {
  ...
  // Açık paneldeki DOM değerlerini node.data'ya flush et
