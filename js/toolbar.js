...
function veShowSaveDialog(defaultName, blob, toastMsg) {
  ...
  modal.style.cssText = 'background:var(--bg-secondary);color:var(--text-primary);border-radius:var(--radius-lg);padding:24px;max-width:420px;width:90%;box-shadow:var(--shadow-xl);border:1px solid var(--border-color);';
  ...
  modal.innerHTML =
    ...
    '<div style="display:flex;align-items:center;gap:0;">' +
      '<input id="ve-save-filename" type="text" value="' + nameOnly.replace(/"/g, '&quot;') + '" ' +
        'style="flex:1;padding:8px 12px;background:var(--bg-tertiary);color:var(--text-primary);border:1px solid var(--border-color);border-radius:var(--radius-sm) 0 0 var(--radius-sm);font-size:var(--fs-lg);outline:none;" />' +
      '<span style="padding:8px 12px;background:var(--bg-tertiary);color:var(--text-muted);border:1px solid var(--border-color);border-left:none;border-radius:0 var(--radius-sm) var(--radius-sm) 0;font-size:var(--fs-lg);white-space:nowrap;">' + ext + '</span>' +
    '</div>' +
...
function veSaveTopology() {
  ...
  // STEP kaynağını YALNIZ BURADA (dosya yolunda) enjekte et. node.data'da
  // taşınmıyor çünkü her `saveState()` onu derin kopyalayıp 50 adımlık undo
  // yığınına bindiriyordu (ÖLÇÜLDÜ: 140 KB dosyada saveState 0,03 → 2,17 ms,
