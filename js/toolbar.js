...
function veExportSummaryTXT() {
  ...
  a.download = 'topoloji_ozeti_' + new Date().toISOString().slice(0,10) + '.txt';
  a.click();
  showToast('Topoloji özeti indirildi');
}
...
function veSaveTopology() {
  ...
  var a = document.createElement('a');
  a.href = url;
  a.download = 'topoloji_' + new Date().toISOString().slice(0,10) + '.json';
  document.body.appendChild(a);
  a.click();
  document.body.removeChild(a);
  URL.revokeObjectURL(url);
  
  showToast('Proje kaydedildi (' + veTabs.length + ' sekme)');
}
