...
var componentDefs = {
  ...
  //
  // KASNAKLAR BAĞLANMAZ — SIRA TABLODA (2026-09-09, kullanıcı isteği).
  // Kayış yolu eskiden kanvasta tel çekilerek kuruluyordu: her kasnak 1 giriş
  // + 1 çıkış taşır, zincir krank çıkışından başlar, krank girişine dönerdi.
  // Artık kasnaklar 0/0 portlu ve serpantin sırası Kayış Tablosu'nun satır
  // sırası (node.data.beltIndex; bkz. js/fead-model.js veFeadBeltOrder).
  //
  // NEDEN: sıra bir GRAF DEĞİL, bir LİSTE. Kapalı tek çevrimde telin
  // taşıyabildiği tek bilgi zaten sıraydı, ama tel onun yanında kurulamayan
  // durumlar da üretiyordu — çatal, kopuk kasnak, kapanmayan çevrim — ve
  // üçünün de tek çaresi kullanıcının teli doğru çekmesiydi. Listede bu
  // hataların hiçbiri KURULAMIYOR.
  //
  // Kasnak kutuları kanvasta DURUYOR: konum kayış düzlemidir (1 px = 1 mm) ve
  // kutuya tıklamak detay hesapların panelini açar. Kalkan tel, kutu değil.
  //
  // "kaynak" ile "yük" ayrımı porttan değil tipten okunur (isFeadDriver /
  // isFeadAccessory). Sarım açısı ve kayış boyu sıradan + konumlardan türer.
  //
  ...
  // ── KAYIŞ YOLU KANVASI — kasnak konumlarından ölçekli serpantin şeması ───
  //
  // Sembolün kendisi de GERÇEK geometridir: üç kasnağın (R=15/9/7) dış
  // teğetleri ve sarım yayları analitik çözülüp path'e yazıldı (cp-fead.js).
  //
  // TEK TİP. Bir dönem İKİ tip vardı — geometri kartı (sarım açıları, gergi
  // kolu) ve çalışma noktası kartı (gerilme haritası, animasyon, titreşim) —
  // ve fark TİPİN İÇİNE gömülüydü: hangi katmanlar açık, hangi seçiciler
  // görünür, devir okunur mu. Katmanlar kart başına seçilebilir olunca
  // (2026-09-11, `node.data.kat`) o ayrım aynı işi İKİNCİ KEZ yapmaya
  // başladı. Kullanıcı bildirimi: *"iki kanvas var, ikisinin de özellikleri
  // falan farklı… Tek kanvas olacak, açılır açılmaz iki kanvas gelsin fakat
  // tipoloji tek olacak."* Tip kalktı; ayrım ÖN AYAR oldu (`data.katOn`:
  // geometri / işletme) ve açılışta yine İKİ kart gelir — ikisi de bu tipten.
  //
  // maxInstances YOK: iki kart olağan kullanım, üçüncüsü de kullanıcının
  // hakkı (rölanti · 2000 dev/dk · geometri yan yana).
  //
  // TİP ADI DEĞİŞMEDİ, KART ADI DEĞİŞEBİLİR. İki kart yan yana dururken ikisi
  // de "Kayış Yolu" etiketi taşısaydı ayırt edilemezlerdi; kurucular ikinci
  // kanvasa `customName` yazıyor ("Çalışma Noktası"). Ön ayarı değiştirmek adı
...
function defaultPortSide(node, portType){
  ...
  // FEAD'in "kenar komşuya bakar" kancası KALKTI (2026-09-09): kasnaklar artık
  // 0/0 portlu, aralarında tel yok. Kanca kalsaydı hiç çağrılmayan bir dal
  // olurdu — ve bir sonraki oturum onu "kullanılıyor" sanırdı.
