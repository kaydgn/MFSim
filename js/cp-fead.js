...
function veFeadKartBolgeW(yer){
  var t = 0;
  VE_FEAD_TABLE_COLS.forEach(function(c){ if(c.yer === yer) t += c.w; });
  return t;
}
function _feadKol(k){
...
function veFeadTableCardHTML(node){
  ...
  h += '<div class="ve-fead-krt-wrap" onmousedown="event.stopPropagation();"'
    // ÇÖZÜM BÖLGESİNİN GENİŞLİĞİ YAZILMAZ: o bölge `1fr` (ARTAN NE İSE O).
    // Yazılsaydı CSS'te kullanılmayan bir sayı dolaşırdı ve bir sonraki okuyan
    // onu kaynak sanardı. Kart varsayılan ölçüsündeyken artan zaten sütun
    // listesinin verdiği genişliğin ta kendisi — kapı bunu tutuyor.
