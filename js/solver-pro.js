    ...
    setTimeout(function() {
      ...
      try {
        ...
        if(veTabs.length > 1) {
          ...
          // Aktif topolojinin global değişkenlerini yedekle
          var _origNodes = nodes;
          var _origConns = connections;
          var _origSimResults = window.veSimResults;
          var _origChartViews = JSON.parse(JSON.stringify(veChartViews));
          ...
          for(var _ti = 0; _ti < veTabs.length; _ti++) {
            if(_ti === veActiveTabIdx) {
              log('[' + (_ti+1) + '/' + veTabs.length + '] ' + veTabs[_ti].name + ' — zaten çözüldü ✓', 'ok');
              continue;
            }
            var _otherTab = veTabs[_ti];
            if(!_otherTab.state || !_otherTab.state.nodes || _otherTab.state.nodes.length === 0) {
              log('[' + (_ti+1) + '/' + veTabs.length + '] ' + _otherTab.name + ' — boş topoloji, atlandı', 'warn');
              continue;
            }

            // Motor/Motor Freni bileşeni var mı kontrol et
            var _hasEngine = _otherTab.state.nodes.some(function(n) { return n.type === 'engine' || n.type === 'engine-brake'; });
            if(!_hasEngine) {
              log('[' + (_ti+1) + '/' + veTabs.length + '] ' + _otherTab.name + ' — motor bileşeni yok, atlandı', 'warn');
              continue;
            }

            log('[' + (_ti+1) + '/' + veTabs.length + '] ' + _otherTab.name + ' çözülüyor...', 'info');

            try {
              // Tab'ın node'larını geçici olarak yükle (DOM oluşturmadan)
              ...
              // Global değişkenleri geçici olarak değiştir
              nodes = _tempNodes;
              ...
              var _otherResult;
              if(veActiveModule === 'full-throttle') {
                var _trN2 = _tempNodes.find(function(n) { return n.type === 'transfer'; });
                var _trd2 = _trN2 ? (_trN2.data || {}) : {};
                var _ftTrG2 = _trd2.ftTrGears || [
                  { kademe: 'High', ratio: 1.054, eff: 97.00 }
                ];
                var _allRR2 = {};
                _ftTrG2.forEach(function(g) {
                  _allRR2[g.kademe] = veFTRunSimulationEngine(g.kademe);
                });
                _otherResult = _allRR2[_ftTrG2[0].kademe];
              } else {
                _otherResult = veRunSimulationEngine();
              }

              // Sonuçları tab state'ine kaydet
