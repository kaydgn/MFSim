...
function getTransferPropertiesHTML(node) {
  ...
  if(isFullThrottle) {
    ...
    html += '<select id="ve-fttr-preset-' + node.id + '" onchange="onVEFTTransferPresetSelect(\'' + node.id + '\', this.value)" style="flex:1; font-size:var(--fs-body); padding:4px 6px; background:var(--bg-input); color:var(--text-primary); border:1px solid var(--border-color); border-radius:var(--radius-sm);">';
  ...
  html += '<select id="ve-transfer-brand-' + node.id + '" onchange="veUpdateTransferModels(\'' + node.id + '\')" style="flex:1; font-size:var(--fs-body); padding:4px 4px; background:var(--bg-input); color:var(--text-primary); border:1px solid var(--border-color); border-radius:var(--radius-sm);">';
  ...
  html += '</select>';
  html += '<select id="ve-transfer-model-' + node.id + '" onchange="veLoadTransferModel(\'' + node.id + '\')" style="flex:1; font-size:var(--fs-body); padding:4px 4px; background:var(--bg-input); color:var(--text-primary); border:1px solid var(--border-color); border-radius:var(--radius-sm);">';
...
function getDifferentialPropertiesHTML(node) {
  ...
  if(isFullThrottle) {
    ...
    if(!isMaster) {
      var html = '<div class="sw-panel">';
      html += '<div class="sw-section-title" style="display:flex;align-items:center;justify-content:space-between;">Diferansiyel <span style="background:var(--bg-tertiary); color:var(--text-muted); font-size:var(--fs-micro); font-weight:600; padding:1px 5px; border:1px solid var(--border-color);">SLAVE</span></div>';
