...
// Smoothing için NODE-konumu yüzey projektörü: bir (x,y,z) noktasını geometrinin
// analitik yüzeyine en yakın noktaya projekte eder (küre/silindir/koni). Boundary
// düğümlerinin tanjant kayması için kullanılır (ANSYS surface smoothing). Yapısal
// _veFEABuildSurfaceProjector'dan farkı: edge değil, tek node konumu alır ve
// "düğüm zaten yüzeyde mi" toleransı içerir (iç düğümler etkilenmez).
// Dönüş: function(x,y,z,nodeId) → [x,y,z] | null  ya da  null (desteklenmeyen tip).
function _veFEABuildNodeSurfaceProjector(geometry, mesh) {
  ...
  if (type === 'sphere' || type === 'hemisphere') {
    ...
    if (!R) return null;
    var tol = Math.max(1e-3, R * 0.02);
    return function(x, y, z) {
      var d = Math.sqrt(x*x + y*y + z*z);
      if (d < 1e-9) return null;
      if (Math.abs(d - R) > tol) return null;   // yüzeyde değil → dokunma
      var s = R / d;
      return [x*s, y*s, z*s];
    };
  }
  if (type === 'cylinder') {
    ...
    if (!Rc) return null;
    var tolc = Math.max(1e-3, Rc * 0.02);
    return function(x, y, z) {
      var rr = Math.sqrt(x*x + z*z);
      if (rr < 1e-9) return null;
      if (Math.abs(rr - Rc) > tolc) return null; // yanal yüzeyde değil (kapak vb.)
      var s = Rc / rr;
      return [x*s, y, z*s];                       // y (eksenel) korunur
    };
  }
  if (type === 'cone') {
    ...
    for (var i = 1; i < mesh.nodes.length; i += 3) {
      var yv = mesh.nodes[i];
      if (yv < yMin) yMin = yv;
      if (yv > yMax) yMax = yv;
    }
    ...
    var tolk = Math.max(1e-3, maxR * 0.02);
    return function(x, y, z) {
      var t = (y - yMin) / H;
      ...
      var rr = Math.sqrt(x*x + z*z);
      if (rr < 1e-9) return null;
      if (Math.abs(rr - Ry) > tolk) return null;  // konik yüzeyde değil
      var s = Ry / rr;
      return [x*s, y, z*s];
    };
  }
  return null; // box/ibeam/lbracket/recttube/torus → düz yüzeyli, projeksiyon yok
}
...
// geometry verilirse eğri yüzeyler için surfaceProjector eklenir (tanjant kayma).
function _veFEAResolveSmoothingOpts(opts, geometry, mesh) {
  ...
  // Eğri yüzeyli primitif → boundary düğümleri yüzeyde kaydır (kalite ↑).
  if (cfg.surfaceProjector) {
    resolved.surfaceProjector = cfg.surfaceProjector;
  } else if (geometry && mesh && cfg.surfaceProjector !== false) {
    var proj = _veFEABuildNodeSurfaceProjector(geometry, mesh);
    if (proj) resolved.surfaceProjector = proj;
  }
  return resolved;
}
...
// Kuadratik mid-side düğüm yüzey projektörü. Eğri primitiflerde (küre,
// yarımküre dome, silindir yanal, koni yanal) kenar orta düğümünü analitik
// yüzeye projekte eder; böylece tet10/hex20/wedge15 elemanları eğri yüzeyi
// 2. dereceden gerçekçi temsil eder (düz kenar ortası ~0.5mm sapardı).
//
// Dönüş: projector(midX, midY, midZ, aId, bId) → [x,y,z] | null
//   - null → lineer (düz) konum korunur (düz yüzeyler, iç kenarlar).
//   - Yalnızca İKİ ucu da analitik yüzeyde olan kenarlar projekte edilir;
//     böylece iç hacim kenarları ve düz yüzeyler (kutu) etkilenmez.
// geometry yoksa veya eğri yüzeyli değilse null döner (projeksiyon yok).
function _veFEABuildSurfaceProjector(geometry, mesh) {
  if (!geometry || !geometry.type || !mesh || !mesh.nodes) return null;
  var nodes = mesh.nodes;
  var p = geometry.params || {};
  // Bir düğümün analitik yüzeyde olup olmadığını test eden + üzerine projekte
  // eden yardımcılar tipe göre seçilir. Tolerans yüzey ölçeğine göreli.
  function nx(id) { return nodes[id * 3]; }
  function ny(id) { return nodes[id * 3 + 1]; }
  function nz(id) { return nodes[id * 3 + 2]; }

  if (geometry.type === 'sphere') {
    var R = Math.max(0.5, p.radius || 25);
    var tol = Math.max(1e-4, R * 0.02);
    var onSphere = function (id) {
      var d = Math.sqrt(nx(id) * nx(id) + ny(id) * ny(id) + nz(id) * nz(id));
      return Math.abs(d - R) <= tol;
    };
    return function (mxv, myv, mzv, a, b) {
      if (!onSphere(a) || !onSphere(b)) return null;
      var d = Math.sqrt(mxv * mxv + myv * myv + mzv * mzv);
      if (d < 1e-9) return null;
      var s = R / d;
      return [mxv * s, myv * s, mzv * s];
    };
  }

  if (geometry.type === 'hemisphere') {
    var Rh = Math.max(0.5, p.radius || 25);
    var tolh = Math.max(1e-4, Rh * 0.02);
    // Dome yüzeyi: |pos| ≈ R ve y > 0 (alt düz disk hariç).
    var onDome = function (id) {
      if (ny(id) <= tolh) return false; // alt düz disk → projekte etme
      var d = Math.sqrt(nx(id) * nx(id) + ny(id) * ny(id) + nz(id) * nz(id));
      return Math.abs(d - Rh) <= tolh;
    };
    return function (mxv, myv, mzv, a, b) {
      if (!onDome(a) || !onDome(b)) return null;
      var d = Math.sqrt(mxv * mxv + myv * myv + mzv * mzv);
      if (d < 1e-9) return null;
      var s = Rh / d;
      return [mxv * s, myv * s, mzv * s];
  ...
  if (geometry.type === 'cylinder') {
    var Rc = Math.max(0.1, p.radius || 15);
    var tolc = Math.max(1e-4, Rc * 0.02);
    // Yanal yüzey: eksenden (Y) radyal mesafe ≈ R. Üst/alt kapaklar değil.
    var onLateral = function (id) {
      var rr = Math.sqrt(nx(id) * nx(id) + nz(id) * nz(id));
      return Math.abs(rr - Rc) <= tolc;
    };
    return function (mxv, myv, mzv, a, b) {
      if (!onLateral(a) || !onLateral(b)) return null;
      var rr = Math.sqrt(mxv * mxv + mzv * mzv);
      if (rr < 1e-9) return null;
      var s = Rc / rr;
      return [mxv * s, myv, mzv * s]; // yalnızca radyal projeksiyon (Y korunur)
    };
  }
  ...
  if (geometry.type === 'cone') {
    ...
    var hC = Math.max(0.1, p.height || 60);
    var half = hC / 2;
    var rMax = Math.max(rB, rT);
    var toln = Math.max(1e-4, rMax * 0.03);
    // y → o yükseklikteki beklenen yarıçap (lineer interpolasyon, eksen Y).
    var radiusAt = function (y) {
      var t = (y + half) / hC; if (t < 0) t = 0; else if (t > 1) t = 1;
      return rB + (rT - rB) * t;
    };
    var onLateralCone = function (id) {
      var rr = Math.sqrt(nx(id) * nx(id) + nz(id) * nz(id));
      var rExp = radiusAt(ny(id));
      if (rExp < 1e-6) return false; // apex civarı → projekte etme
      return Math.abs(rr - rExp) <= toln;
    };
    return function (mxv, myv, mzv, a, b) {
      if (!onLateralCone(a) || !onLateralCone(b)) return null;
      var rExp = radiusAt(myv);
      if (rExp < 1e-6) return null;
      var rr = Math.sqrt(mxv * mxv + mzv * mzv);
      if (rr < 1e-9) return null;
      var s = rExp / rr;
      return [mxv * s, myv, mzv * s];
    };
  }

  // Düz yüzeyli / desteklenmeyen tipler (kutu, ibeam, vb.) → projeksiyon yok.
  return null;
}
...
function veFEAMeshFromGeometry(geometry, opts) {
  ...
  // Face Sizing kontrolleri (ANSYS §5.1) — eksenel yüzlerde hedef boyuta göre
  // kümeleme bias'ı uygula (B.5: lokal kontrollerin gerçek mesh etkisi).
  // Sadece düğüm kaydırma → mesh her zaman geçerli (eleman ekleme/silme yok).
  // localSizing ile aynı konumda (tet4/quadratic dönüşümünden ÖNCE) ki
  // midpoint düğümleri doğru pozisyonda hesaplansın.
  if (mesh && !mesh.error && Array.isArray(opts.faceSizingControls) && opts.faceSizingControls.length) {
    mesh = _veFEAApplyFaceSizingBias(mesh, opts.faceSizingControls, size);
  }
  ...
  if (mesh && !mesh.error && opts.smoothing === true && typeof veFEASmoothMesh === 'function' &&
    ...
    mesh = veFEASmoothMesh(mesh, _veFEAResolveSmoothingOpts(opts, geometry, mesh));
  }
...
// Face Sizing kontrollerini eksenel kümeleme bias'ına çevir (B.5).
// Mevcut _veFEAApplyFaceSizingControls (named-selection işaretleme) ile AYRI:
// bu fonksiyon GEOMETRİK etki uygular (düğümleri yüze doğru kümeler), tet4/
// quadratic dönüşümünden ÖNCE çağrılır. Sadece düğüm kaydırma → mesh geçerli
// kalır. Eksenel olmayan yüzlerde (silindir yan yüzü, STEP) veFEAApplyLocalSizing
// mesh'i değiştirmeden döner.
function _veFEAApplyFaceSizingBias(mesh, controls, globalSize) {
  if (!mesh || mesh.error || !Array.isArray(controls) || controls.length === 0) return mesh;
  var gs = (isFinite(globalSize) && globalSize > 0) ? globalSize : 10;
  controls.forEach(function(c) {
    if (!c) return;
    var faceName = c.faceId;
    if (faceName == null && Array.isArray(c.faceIds) && c.faceIds.length === 1) faceName = c.faceIds[0];
    if (faceName == null) return;
    var target = Number(c.size);
    if (!isFinite(target) || target <= 0) return;
    var ratio = gs / target;
    if (ratio <= 1.01) return;  // hedef ≥ global → kümeleme gereksiz
    var strength = 0.30 + 0.18 * (ratio - 1);
    if (strength > 0.85) strength = 0.85;
    if (c.behavior === 'hard') strength = Math.min(0.9, strength + 0.1);
    mesh = veFEAApplyLocalSizing(mesh, {
      selection: faceName, biasMode: 'power', biasStrength: strength
    });
  });
  return mesh;
}
  ...
  for (var pi = 0; pi < prim.length; pi++) {
    for (var si = 0; si < sizes.length; si++) {
      ...
      try {
        ...
        // ANSYS-tarzı genel kalite skoru (0-100) — görünürlük için
        if (typeof veFEAComputeMeshQualityScore === 'function') {
          var qsc = veFEAComputeMeshQualityScore(m);
          row.score = qsc ? qsc.score : null;
          row.grade = qsc ? qsc.grade : null;
        }
    ...
    if (console.table) {
      console.table(rows.map(function (r) {
        return { Senaryo: r.senaryo, Tip: r.tip, Eleman: r.eleman, Inverted: r.inverted,
                 ...
                 Skor: (r.score != null ? r.score + '/' + r.grade : '–'),
                 Sonuç: r.ok ? '✓' : '✗ ' + r.not };
      }));
    } else {
