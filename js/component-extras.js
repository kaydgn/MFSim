...
function getSensorPropertiesHTML(node) {
  ...
  html += '<div style="font-size:var(--fs-lg); font-weight:600; color:#22c55e; margin-bottom:8px;"><span class="mf-ico mf-ico-map-pin"></span> Sensör</div>';
  ...
  if(!conn && !compNode) {
    ...
    availableSignals.forEach(function(sig) {
      ...
      html += '<label style="display:flex; align-items:center; gap:4px; font-size:var(--fs-tiny); padding:2px 0; cursor:pointer; color:' + (isChecked ? '#22c55e' : 'var(--text-secondary)') + ';">';
    ...
    html += '<tr style="border-bottom:1px solid var(--border-color);"><td colspan="2" style="padding:4px 8px; font-size:var(--fs-micro); color:var(--text-muted); background:var(--bg-secondary);">';
    ...
    html += '<div style="font-size:var(--fs-micro); color:var(--text-muted);"><span class="mf-ico mf-ico-bar-chart"></span> Okunabilir Sinyaller (' + dirLabel + '): <b>' + availableSignals.length + '</b> sinyal</div>';
    html += '</td></tr>';
...
function getObstacleCrossingPropertiesHTML(node) {
  ...
  html += '<td style="padding:6px 8px; background:var(--bg-secondary); color:var(--text-primary); font-weight:500; text-align:right; font-size:var(--fs-tiny);">' + autoTireName + ' <span style="font-size:var(--fs-micro); color:var(--text-muted); font-weight:400;">( Lastik )</span></td>';
  html += '</tr>';
...
function veOpenCoastDownWizard(nodeId) {
  ...
  var pThStyle = 'padding:4px 8px; font-size:var(--fs-body); font-weight:500; color:var(--text-secondary); text-align:left; white-space:nowrap; width:45%;';
  var pTdStyle = 'padding:4px 8px;';
  var pInpStyle = 'width:80px; padding:3px 6px; font-size:var(--fs-body); background:var(--bg-input); color:var(--text-primary); border:1px solid var(--border-color); border-radius:var(--radius-sm); text-align:right;';
  ...
  var thInp = 'padding:6px 5px; border-bottom:1px solid var(--border-color); color:var(--text-secondary); font-weight:600; font-size:var(--fs-md); text-align:center;';
  var thCalc = 'padding:6px 10px; border-bottom:1px solid var(--border-color); color:var(--text-muted); font-weight:500; font-size:var(--fs-md); text-align:center;';
  ...
  var tableHtml = '<div style="margin-bottom:14px; border:1px solid var(--border-color); border-radius:var(--radius-sm); overflow:hidden;">' +
    ...
    '<button onclick="cdwClearRows()" style="padding:3px 10px; font-size:var(--fs-body); font-weight:600; background:var(--bg-secondary); color:var(--text-secondary); border:1px solid var(--border-color); border-radius:var(--radius-sm); cursor:pointer;"><span class="mf-ico mf-ico-trash"></span> Sil</button>' +
    '</div></div>' +
  ...
  var summaryHtml = '<div style="margin-bottom:14px; border:1px solid var(--border-color); border-radius:var(--radius-sm); overflow:hidden;">' +
    ...
    '<div style="display:grid; grid-template-columns:1fr 1fr; gap:4px 24px; font-size:var(--fs-body); color:var(--text-secondary); margin-bottom:8px;">' +
...
function cdwAddRow() {
  ...
  var inpStyle = 'width:84px; padding:5px 6px; font-size:var(--fs-md); background:var(--bg-input); color:var(--text-primary); border:1px solid var(--border-color); border-radius:var(--radius-sm); text-align:right;';
  ...
  tr.innerHTML = '<td style="padding:4px 3px; text-align:center; width:22px;"><button onclick="this.closest(\'tr\').remove()" style="background:none; border:none; color:var(--accent-danger); cursor:pointer; font-size:var(--fs-body); padding:0; opacity:0.4;" onmouseover="this.style.opacity=\'1\'" onmouseout="this.style.opacity=\'0.4\'">✕</button></td>' +
    ...
    '<td style="' + tdCalc + '"><span class="cdw-crr" style="font-size:var(--fs-md); font-weight:700; color:#4ade80;"></span></td>';
...
function getWheelPropertiesHTML(node) {
  ...
  if(isFullThrottle) {
    ...
    html += '<select id="ve-ft-tire-preset-' + node.id + '" onchange="onVEFTTirePresetChange(\'' + node.id + '\', this.value)" style="flex:1; font-size:var(--fs-body); padding:4px 6px; background:var(--bg-input); color:var(--text-primary); border:1px solid var(--border-color); border-radius:var(--radius-sm);">';
  ...
  html += '<div style="display:flex; gap:6px; margin-bottom:8px; align-items:center;">';
  html += '<select id="ve-tire-preset-' + node.id + '" onchange="onVETirePresetChange(\'' + node.id + '\', this.value)" style="flex:1; font-size:var(--fs-body); padding:4px 6px; background:var(--bg-input); color:var(--text-primary); border:1px solid var(--border-color); border-radius:var(--radius-sm);">';
  ...
  html += '<tr style="border-bottom:1px solid var(--border-color);">';
  html += '<td colspan="2" style="padding:6px 8px; font-size:var(--fs-tiny); color:var(--text-secondary); background:var(--bg-secondary); line-height:1.4;">Dinamik tekerlek yarıçapı. Lastik boyutundan hesaplanır veya ölçülür.</td>';
  html += '</tr>';
  ...
  html += '<tr>';
  html += '<td colspan="2" style="padding:6px 8px; font-size:var(--fs-tiny); color:var(--text-secondary); background:var(--bg-secondary); line-height:1.4;">Dönen parçaların (tekerlek, şaft, motor) etkili kütle artışı. Tipik: 1.05–1.15.</td>';
  html += '</tr>';
...
function getVehiclePropertiesHTML(node) {
  ...
  html += '<span style="font-size:var(--fs-body);color:var(--text-primary);">Aktif</span>';
  html += '</label>';
  html += '</td>';
  html += '</tr>';
  html += '<tr style="border-bottom:1px solid var(--border-color);">';
  html += '<td colspan="2" style="padding:5px 8px; font-size:var(--fs-tiny); color:var(--text-muted); background:var(--bg-secondary); line-height:1.3;">İşaretlenirse, motor devri RPM eşiğine ulaştığında otomatik vites değişimi simüle edilir. 1. ve 2. viteslerde motor freni devre dışıdır.</td>';
  html += '</tr>';
...
function veManualSegExpandProfile(nodeId) {
  ...
  // Footer
  var footer = document.createElement('div');
  footer.style.cssText = 'padding:6px 14px; background:var(--bg-tertiary); border-top:1px solid var(--border-color); flex-shrink:0; font-size:var(--fs-micro); color:var(--text-muted); text-align:center;';
