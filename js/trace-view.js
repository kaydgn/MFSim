...
function veTrGeometry(ctx, lanes, view, w, availH) {
  ...
  //
  // OLUKTA YALNIZCA SAYILAR DURUR — eksen ADLARI şeridin içindeki lejanttadır
  // (veTrDrawAxisLegend). Ad şeridini eksen başına oluğa koymak denendi ve
  // ÖLÇÜLDÜ: dört eksende oluk 215 px'e çıkıyor, 700 px'lik bir pencerede üst
  // sınıra takılıyor ve en dıştaki eksen tuvalin dışına (x = −1,5) düşüp
  // tamamen görünmez oluyordu. Ad yatay bir lejantta çok daha az yer kaplıyor
  // ve şeridin içinde bol boşluk var.
  ...
  lanes.forEach(function(lane) {
    ...
    (lane.axes || []).forEach(function(a) {
      ...
      a._tickX = gutter - off;
      a._fits = (a._labelX - a._w) >= VE_TR.TITLE_BAND;
...
// ── Çizim: birleşik şeridin eksen lejantı ────────────────────────────────────
//
// Çok eksenli şeritte hangi sayının hangi eğriye ait olduğu SORULMAMALI. Oluğa
// eksen başına dikey ad şeridi koymak denendi ve ölçüldü: dört eksende oluk
// 215 px'e çıkıyor, dar pencerede en dıştaki eksen tuvalin dışına düşüyordu.
// Ad yatay yazıldığında aynı bilgi çok daha az yer kaplıyor ve şeridin
// içindeki boşluğa sığıyor.
//
// Sıra EKSEN SIRASI ile aynı: soldaki ad, oluktaki en içteki (çizim alanına
// en yakın) sütuna karşılık gelir. Böyle olmasaydı lejant eşlemeyi çözmek
// yerine yeni bir bilmece eklerdi.
function veTrDrawAxisLegend(ctx, geo, lane, rect) {
  var axes = lane.axes || [];
  ...
  var DOT = 6, GAP_IN = 4, GAP_OUT = 11, PAD = 4;
  var items = axes.map(function(a) {
    // Sığmayan eksen SESSİZ kalmamalı: sayıları çizilmedi, sebebi burada yazılı.
    var t = veTrAxisTitle(a) + (a._fits === false ? ' (oluğa sığmadı)' : '');
    return { color: a.color, text: t, w: DOT + GAP_IN + ctx.measureText(t).width };
  });

  var total = items.reduce(function(s, it) { return s + it.w; }, 0)
              + GAP_OUT * (items.length - 1);
  var maxW = geo.plotW - 16;

  // Sığmıyorsa SONDAN kırp ve kaç tanesinin düştüğünü yaz. Sessizce taşırmak
  // lejantı eğrilerin üstüne yayardı.
  var shown = items, extra = 0;
  if(total > maxW) {
    shown = []; var acc = 0;
    for(var i = 0; i < items.length; i++) {
      var next = acc + items[i].w + (shown.length ? GAP_OUT : 0);
      if(next > maxW - 34) break;
      shown.push(items[i]); acc = next;
    }
    extra = items.length - shown.length;
    total = acc + (extra ? GAP_OUT + 30 : 0);
  }

  var x = geo.plotX + 8;
  var y = rect.y + 9;

  // Zemin: eğri lejantın altından geçtiğinde yazı okunmaz kalmasın.
  ctx.fillStyle = (typeof veThemeRgba === 'function')
    ? veThemeRgba('--bg-primary', 0.82, 'rgba(20,22,28,0.82)') : 'rgba(20,22,28,0.82)';
  ctx.fillRect(x - PAD, y - 7, Math.min(total, maxW) + PAD * 2, 14);

  shown.forEach(function(it, i) {
    if(i) x += GAP_OUT;
    ctx.fillStyle = it.color;
    ctx.fillRect(x, y - 2.5, DOT, 5);
    x += DOT + GAP_IN;
  ...
  if(extra) {
    ...
    ctx.fillText('+' + extra, x + GAP_OUT, y);
  }
...
function veTrDrawLane(ctx, geo, lane, rect, idx, timeArr, xTicks) {
  ...
  // Adların tamamı şeridin içindeki lejantta (hemen aşağıda).
  if(axes.length <= 1) {
    ctx.save();
    ...
    veTrDrawAxisLegend(ctx, geo, lane, rect);
  }
