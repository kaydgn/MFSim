...
function veGenerateFTTxtReport(sim, optHazirlayan) {
  ...
  r += '  ALAN VE AGIRLIK' + pad('', 22) + 'LASTIKLER\n';
  r += '  ' + ln('-', 38) + '  ' + ln('-', 38) + '\n';
  r += pad(pRow('Alin Alani', num(R.frontalArea, 3) + ' m2'), 40) + pRow('Secili Lastik', ascii(R.tireName));
  r += pad(pRow('Yukseklik / Genislik', num(R.height, 3) + ' / ' + num(R.width, 3) + ' m'), 40) + pRow('Lastik Devir/km', revPerKm + ' devir/km');
  r += pad(pRow('Aerodinamik Direnc (Cd)', num(R.cd, 3)), 40) + pRow('Lastik Yuvarlanma Yari.', num(R.tireRadius, 3) + ' m');
  r += pad(pRow('Brut Arac Agirligi (GVW)', numI(R.gvw) + ' kg'), 40) + pRow('Yuvarlanma Direnci (Crr)', num(R.crr, 4));
  r += pad('', 40) + pRow('Yuzey Faktoru', num(R.surfFactor || 1.0, 2));
  r += pad('', 40) + pRow('Lastik/Tekerlek Ataleti', num(R.tireInertia, 4) + ' kg.m2');
  r += '\n\n';
