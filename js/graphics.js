...
// ── Y-Axis Manuel Kontrol ──
function veSetAxisLock(slotIdx) {
  var slot = veResultSlots[slotIdx];
  if(!slot) return;
  if(!slot.yAxisLock) slot.yAxisLock = {};
  
  var minEl = document.getElementById('ve-ymin-' + slotIdx);
  var maxEl = document.getElementById('ve-ymax-' + slotIdx);
  var minREl = document.getElementById('ve-ymin-r-' + slotIdx);
  var maxREl = document.getElementById('ve-ymax-r-' + slotIdx);
  
  // Sol eksen
  slot.yAxisLock.min = (minEl && minEl.value !== '') ? parseFloat(minEl.value) : undefined;
  slot.yAxisLock.max = (maxEl && maxEl.value !== '') ? parseFloat(maxEl.value) : undefined;
  // Sağ eksen
  slot.yAxisLock.minR = (minREl && minREl.value !== '') ? parseFloat(minREl.value) : undefined;
  slot.yAxisLock.maxR = (maxREl && maxREl.value !== '') ? parseFloat(maxREl.value) : undefined;
  
  // Zoom/pan'ı sıfırla (manuel aralık ayarlandığında)
  veChartViews[slotIdx] = { panX: 0, panY: 0, zoomX: 1, zoomY: 1 };
...
function veClearAxisLock(slotIdx) {
  ...
  var ids = ['ve-ymin-', 've-ymax-', 've-ymin-r-', 've-ymax-r-'];
  ids.forEach(function(id) {
    var el = document.getElementById(id + slotIdx);
    if(el) el.value = '';
  });
...
function veRenderChart(slotIdx) {
  ...
  // Zaman ekseni: sensörlerin kaynak simResults'larından en uzununu bul
  var r = window.veSimResults;
  var timeArr = (r && r.time) ? r.time : null;
  ...
  slot.sensors.forEach(function(s) {
    if(s.id.charAt(0) === '@') {
      ...
      if(tr && tr.time && (!timeArr || tr.time.length > timeArr.length)) {
        timeArr = tr.time;
        r = tr; // en uzun time dizisinin kaynağını kullan
      }
    }
  });
  ...
  // ====== Y-AXIS LOCK OVERRIDE ======
  var yLock = slot.yAxisLock || {};
  // Sol eksen (ilk grup) — yLock.min / yLock.max
  if(unitGroups.length > 0) {
    if(yLock.min !== undefined && isFinite(yLock.min)) unitGroups[0].yMin = yLock.min;
    if(yLock.max !== undefined && isFinite(yLock.max)) unitGroups[0].yMax = yLock.max;
    unitGroups[0]._locked = (yLock.min !== undefined || yLock.max !== undefined);
  }
  // Sağ eksen (ikinci grup) — yLock.minR / yLock.maxR
  if(unitGroups.length > 1) {
    if(yLock.minR !== undefined && isFinite(yLock.minR)) unitGroups[1].yMin = yLock.minR;
    if(yLock.maxR !== undefined && isFinite(yLock.maxR)) unitGroups[1].yMax = yLock.maxR;
    unitGroups[1]._locked = (yLock.minR !== undefined || yLock.maxR !== undefined);
  }
  
  // En fazla 2 eksen — fazlası sol eksene eklenir
  var hasDualAxis = unitGroups.length >= 2;
  var leftAxis = unitGroups[0]; // sol Y ekseni (ilk birim grubu)
  var rightAxis = hasDualAxis ? unitGroups[1] : null; // sağ Y ekseni
  // 3+ farklı birim varsa → hepsini sol eksene ekle (fallback)
  if(unitGroups.length > 2) {
    for(var ug = 2; ug < unitGroups.length; ug++) {
      leftAxis.dsIndices = leftAxis.dsIndices.concat(unitGroups[ug].dsIndices);
      if(unitGroups[ug].yMin < leftAxis.yMin) leftAxis.yMin = unitGroups[ug].yMin;
      if(unitGroups[ug].yMax > leftAxis.yMax) leftAxis.yMax = unitGroups[ug].yMax;
    }
    unitGroups = [leftAxis, rightAxis];
  }
  
  // Dataset → axis mapping
  datasets.forEach(function(ds, di) {
    ds._axisIdx = 0; // default left
    if(rightAxis && rightAxis.dsIndices.indexOf(di) >= 0) ds._axisIdx = 1;
  });
  
  // ====== MARJİN ======
  var margin = {left: 58, right: hasDualAxis ? 58 : 14, top: 14, bottom: 30};
  ...
  var axes = [leftAxis];
  if(rightAxis) axes.push(rightAxis);
  ...
  slot._chartMeta = {
    ...
    xMin: xMin, xMax: xMax,
    yMin: leftAxis._viewMin, yMax: leftAxis._viewMax,
    dataXMin: dataXMin, dataXMax: dataXMax,
    dataYMin: leftAxis.yMin, dataYMax: leftAxis.yMax,
    datasets: datasets,
    timeArr: timeArr,
    hasDualAxis: hasDualAxis,
    axes: axes.map(function(ax) {
      return { unit: ax.unit, yMin: ax._viewMin, yMax: ax._viewMax, yStep: ax._yStep };
    })
  };
  ...
  // Y grid (sol eksen referanslı)
  var lyStep = leftAxis._yStep;
  for(var yv = leftAxis._tickStart; yv <= leftAxis._viewMax + lyStep * 0.01; yv += lyStep) {
  ...
  var leftColor = hasDualAxis ? datasets[leftAxis.dsIndices[0]].color : 'rgba(160,160,180,0.8)';
  for(var yv = leftAxis._tickStart; yv <= leftAxis._viewMax + lyStep * 0.01; yv += lyStep) {
    var gy = yPosAxis(yv, 0);
    ...
    ctx.textAlign = 'right';
    ctx.fillText(veFormatAxisVal(yv), margin.left - 6, gy);
  }
  // Sol birim etiketi
  if(hasDualAxis && leftAxis.unit) {
    ctx.save();
    ...
    ctx.fillText('[' + leftAxis.unit + ']', 0, 0);
    ctx.restore();
  }
  
  // ====== SAĞ Y ETİKETLERİ (dual axis) ======
  if(hasDualAxis && rightAxis) {
    var ryStep = rightAxis._yStep;
    var rightColor = datasets[rightAxis.dsIndices[0]].color;
    for(var yv = rightAxis._tickStart; yv <= rightAxis._viewMax + ryStep * 0.01; yv += ryStep) {
      var gy = yPosAxis(yv, 1);
      if(gy < margin.top - 2 || gy > margin.top + ph + 2) continue;
      ctx.fillStyle = rightColor;
      ctx.font = '10px -apple-system,system-ui,sans-serif';
      ctx.textAlign = 'left';
      ctx.fillText(veFormatAxisVal(yv), margin.left + pw + 6, gy);
    }
    // Sağ birim etiketi
    if(rightAxis.unit) {
      ctx.save();
      ctx.translate(w - 8, margin.top + ph / 2);
      ctx.rotate(Math.PI / 2);
      ctx.fillStyle = rightColor;
      ctx.font = 'bold 9px -apple-system,system-ui,sans-serif';
      ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
      ctx.fillText('[' + rightAxis.unit + ']', 0, 0);
      ctx.restore();
    }
    
    // Sağ eksen çerçeve
    ctx.strokeStyle = rightColor + '40';
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(margin.left + pw, margin.top);
  ...
  // Sağ eksen kontrol panelini göster/gizle
  var rCtrl = document.getElementById('ve-yaxis-r-ctrl-' + slotIdx);
  if(rCtrl) rCtrl.style.display = hasDualAxis ? '' : 'none';
}
...
function veChartShowTooltip(slotIdx, e) {
  ...
  m.datasets.forEach(function(ds) {
    ...
    // Eksen tarafı göstergesi (dual modda)
    var axisBadge = '';
    if(m.hasDualAxis) {
      axisBadge = axIdx === 0 ? ' <span style="opacity:0.4;font-size:0.55rem;">◀</span>' : ' <span style="opacity:0.4;font-size:0.55rem;">▶</span>';
    }
...
function veRenderTable(slotIdx) {
  ...
  // Zaman dizisi: aktif veya cross-tab kaynaktan
  var r = window.veSimResults;
  var timeArr = (r && r.time) ? r.time : null;
