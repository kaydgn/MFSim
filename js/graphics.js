  ...
  if(_pFinalDec && _pTrq && _pStl && _pStl.hasData && isFinite(_pFinalDec.T_req_rear)) {
    ...
    if(tekTarafGecti) {
      ...
      if(Math.abs(_deltaA2) > 0.001) {
        ...
        r += '  Referans: Arka dingil\n\n';

        var _yukler = [200, 500, 1000, 1500, 2000];
        r += '    ' + pad('Yuk (kg)', 12) + pad('Tasima (m)', 14) + 'Yon\n';
        r += '    ' + ln('-', 38) + '\n';
        for(var _yi = 0; _yi < _yukler.length; _yi++) {
          var _yP = _yukler[_yi];
          var _yD = _pMass * _deltaA2 / _yP;
          var _yYon = _yD > 0 ? 'one' : 'arkaya';
          r += '    ' + pad(_yP.toString(), 12) + pad(num(Math.abs(_yD), 2), 14) + _yYon + '\n';
        }
        r += '\n';

        // En pratik öneri bul (1-3m arası taşıma)
