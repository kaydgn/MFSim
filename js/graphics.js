...
function veGenerateTopologyTxtReport(optHazirlayan) {
  ...
  // ═══ TOPOLOJI BILESEN LISTESI ═══
  r += ln('=', W) + '\n';
  r += pad('TOPOLOJI BILESEN LISTESI', W, 'center') + '\n';
  r += ln('=', W) + '\n\n';
  var tblW = 68;
  r += '  ' + pad('No', 4) + pad('Tip', 22) + pad('Ad', 28) + pad('Konum (x,y)', 14, 'right') + '\n';
  r += '  ' + ln('-', tblW) + '\n';
  nodes.forEach(function(n, i) {
    var tipAd = (componentDefs[n.type] ? componentDefs[n.type].name : n.type);
    var ad = n.customName || tipAd;
    r += '  ' + pad(String(i + 1), 4) + pad(ascii(tipAd), 22) + pad(ascii(ad), 28) + pad(numI(n.x) + ',' + numI(n.y), 14, 'right') + '\n';
  });
  r += '  ' + ln('-', tblW) + '\n\n\n';
  ...
  if (engineNode) {
    ...
    if (tData.length > 0) {
      ...
      r += '  MOTOR TORK / GUC VERISI\n';
      r += '  ' + ln('-', 40) + '\n';
      r += '  ' + pad('Devir', 9, 'right') + pad('Tork', 14, 'right') + pad('Guc', 14, 'right') + '\n';
      r += '  ' + pad('[rpm]', 9, 'right') + pad('[N.m]', 14, 'right') + pad('[kW]', 14, 'right') + '\n';
      r += '  ' + ln('-', 40) + '\n';
      tData.forEach(function(p) {
        var rpm = p.rpm || p.x || 0;
        var torque = p.torque || p.y || 0;
        var pw = p.power || (torque * rpm * Math.PI / 30000);
        var tag = '';
        if (Math.abs(rpm - pkTr) < 5) tag = '  Pik Tork';
        else if (Math.abs(rpm - pkPr) < 5) tag = '  Pik Guc';
        else if (ms.governedSpeed && Math.abs(rpm - ms.governedSpeed) < 5) tag = '  Governed';
        r += '  ' + pad(numI(rpm), 9, 'right') + pad(num(torque, 1), 14, 'right') + pad(num(pw, 1), 14, 'right') + tag + '\n';
      });
      r += '  ' + ln('-', 40) + '\n';
    }
    ...
    if (ed.accessories && ed.accessories.length > 0) {
      r += '\n  AKSESUAR KAYIPLARI\n';
      r += '  ' + ln('-', 52) + '\n';
      r += '  ' + pad('Aksesuar', 26) + pad('Std (kW)', 13, 'right') + pad('Kullanici (kW)', 13, 'right') + '\n';
      r += '  ' + ln('-', 52) + '\n';
      var aStd = 0, aUsr = 0;
      ed.accessories.forEach(function(a) {
        r += '  ' + pad(ascii(a.name), 26) + pad(num(a.standardLoss, 1), 13, 'right') + pad(num(a.userLoss, 1), 13, 'right') + '\n';
        aStd += (a.standardLoss || 0);
        aUsr += (a.userLoss || 0);
      });
      r += '  ' + ln('-', 52) + '\n';
      r += '  ' + pad('TOPLAM', 26) + pad(num(aStd, 1), 13, 'right') + pad(num(aUsr, 1), 13, 'right') + '\n';
      r += '  ' + ln('-', 52) + '\n';
    }
    r += '\n\n';
    secNo++;
