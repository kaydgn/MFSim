  ...
  if(node.data.routeSegments) {
    ...
    if(segBilgi) {
      ...
      segBilgi.innerHTML = '<div style="display:flex; gap:8px; flex-wrap:wrap; font-size:0.6rem;">' +
        ...
        '<span style="color:var(--accent-success);">📊 <b>' + segs2.length + '</b> seg.</span>' +
        '<span style="color:var(--accent-warning);">📐 Ort: <b>%' + (node.data.routeAvgGrade || 0).toFixed(1) + '</b></span>' +
        '<span style="color:var(--accent-danger);">⬆️ Max: <b>%' + maxE.toFixed(1) + '</b></span>' +
        '</div>';
    }
  }

  // Profili geri oluştur
...
function veCalcDistGradeProfile(nodeId) {
  ...
  // Rakım + Eğim yükseklik verileri
  var elevData = (node.data && node.data.routeElevations) ? node.data.routeElevations : null;
  var elevFirst = elevData && elevData.length > 0 ? elevData[0].elevation : 0;
  var elevLast = elevData && elevData.length > 0 ? elevData[elevData.length - 1].elevation : 0;
  ...
  function _buildProfileHTML(canvasId) {
    ...
    return (elevData ? (
      ...
      '<span style="color:var(--text-muted); opacity:0.4;">│</span>' +
      '<span id="ve-alt-selection-info-' + nodeId + '" style="color:var(--text-muted); font-style:italic;">Sol tık+sürükle ile eğim ölçün</span>' +
      '</div>'
    ) : '') +
    // ── Eğim Profili ──
  ...
  if(profilesSection && profilesContent) {
    ...
    setTimeout(function() {
      ...
      if(elevData) {
        ...
        veRenderAltitudeProfile(altCanvasId, elevData, nodeId);
      }
      // Eğim profili
...
// ═══ RAKIM PROFİLİ (GPS tarzı yükseklik grafiği) ═══
function veRenderAltitudeProfile(canvasId, elevData, nodeId) {
  var canvas = document.getElementById(canvasId);
  if(!canvas || !elevData || elevData.length < 2) return;
  ...
  // Kümülatif mesafe ve yükseklik dizisi
  var pts = []; // {dist, elev}
  var cumDist = 0;
  pts.push({ dist: 0, elev: elevData[0].elevation });
  for(var i = 1; i < elevData.length; i++) {
    cumDist += veHesaplaMesafe(elevData[i-1].lat, elevData[i-1].lng, elevData[i].lat, elevData[i].lng);
    pts.push({ dist: cumDist, elev: elevData[i].elevation });
  }
  var totalDist = cumDist;
  ...
  // Başlık
  ctx.fillStyle = headColor; ctx.font = '600 10.5px Segoe UI, sans-serif'; ctx.textAlign = 'center';
  ctx.fillText('RAKIM PROFİLİ (GPS)', padL + plotW / 2, 14);
  ...
  if(canvas._dgSelection) {
    ...
    if(selX2 > selX1) {
      ...
      ctx.strokeRect(selX1, padT, selX2 - selX1, plotH);
      ctx.setLineDash([]);
      // Seçili bölgenin başlangıç ve bitiş yükseklikleri arası eğim çizgisi
      if(sel.startElev !== undefined && sel.endElev !== undefined) {
        ctx.strokeStyle = '#ffeb3b';
        ctx.lineWidth = 2;
        ctx.setLineDash([4, 2]);
        ctx.beginPath();
        ctx.moveTo(selX1, toY(sel.startElev));
        ctx.lineTo(selX2, toY(sel.endElev));
        ctx.stroke();
        ctx.setLineDash([]);
        // Eğim etiketi
        ctx.fillStyle = '#ffeb3b';
        ctx.font = '700 9px Segoe UI, sans-serif';
        ctx.textAlign = 'center';
        ctx.fillText('Eğim: %' + sel.avgGrade.toFixed(2) + '  Δh: ' + sel.deltaH.toFixed(1) + 'm', (selX1 + selX2) / 2, Math.min(toY(sel.startElev), toY(sel.endElev)) - 6);
      }
    }
  }
  ...
  // Etkileşim verisi
  canvas._drChart = {
    type: 'altProfile', pts: pts, elevData: elevData,
    padL: padL, padR: padR, padT: padT, padB: padB,
    plotW: plotW, plotH: plotH, xMin: xMin, xMax: xMax, yMin: yMin, yMax: yMax,
    baseXMin: baseXMin, baseXMax: baseXMax, baseYMin: baseYMin, baseYMax: baseYMax,
    fromX: fromX, fromY: fromY, W: W, H: H, totalDist: totalDist,
  ...
  if(!canvas._drEventsAttached) {
    ...
    // ── Bölge seçimi (sol tık + sürükle) — eğim hesabı ──
    canvas.addEventListener('mousedown', function(e) {
      if(e.button !== 0) return;
      var d = canvas._drChart;
    ...
    canvas.addEventListener('mousemove', function(e) {
      ...
      var mx = e.clientX - rect.left;
      var xVal = d.fromX(mx);
      var x1 = Math.max(0, Math.min(canvas._dgSelStartX, xVal));
      var x2 = Math.min(d.totalDist, Math.max(canvas._dgSelStartX, xVal));
      if(x2 - x1 < 1) return;

      // Seçimin başlangıç ve bitiş yüksekliklerini bul (interpolasyon)
      function interpElev(dist) {
        for(var k = 0; k < d.pts.length - 1; k++) {
          if(dist >= d.pts[k].dist && dist <= d.pts[k+1].dist) {
            var t = (dist - d.pts[k].dist) / (d.pts[k+1].dist - d.pts[k].dist);
            return d.pts[k].elev + t * (d.pts[k+1].elev - d.pts[k].elev);
          }
        }
        return d.pts[d.pts.length - 1].elev;
      }
      var e1 = interpElev(x1), e2 = interpElev(x2);
      var dh = e1 - e2; // pozitif = iniş
      var selDist = x2 - x1;
      var selGrade = selDist > 0.1 ? (dh / selDist) * 100 : 0;

      canvas._dgSelection = { xStart: x1, xEnd: x2, startElev: e1, endElev: e2, deltaH: dh, avgGrade: selGrade, dist: selDist };
      _drRedrawChart(canvas);

      // Bilgi kutusunu güncelle
      var nid = d.nodeId;
      if(nid) {
        var selEl = document.getElementById('ve-alt-selection-info-' + nid);
        if(selEl) {
          selEl.innerHTML = '<span style="color:#ffeb3b; font-weight:700;">Eğim: %' + selGrade.toFixed(2) + '</span>' +
            '<span style="color:var(--text-muted); margin-left:6px;">Δh: ' + dh.toFixed(1) + 'm (' + (selDist / 1000).toFixed(2) + ' km)</span>';
          selEl.style.fontStyle = 'normal';
        }
      }
    });
...
function veExpandProfileChart(nodeId, chartType) {
  ...
  if(isAlt && (!node.data.routeElevations || node.data.routeElevations.length < 2)) {
    showToast('Rakım verisi bulunamadı', 'warning'); return;
  }
  ...
  header.innerHTML = '<span style="font-size:0.82rem; font-weight:700; color:var(--text-heading);">' + headerTitle + '</span>' +
    ...
    '<span style="font-size:0.56rem; color:var(--text-muted);">Scroll: Zoom &nbsp;│&nbsp; ' + (isAlt ? 'Sol Tık+Sürükle: Eğim Ölç &nbsp;│&nbsp; ' : '') + 'Sağ Tık+Sürükle: Kaydır &nbsp;│&nbsp; Çift Tık: Sıfırla</span>' +
  ...
  setTimeout(function() {
    ...
    if(expandCanvas) {
      ...
      if(isAlt) {
        veRenderAltitudeProfile(expandCanvasId, node.data.routeElevations, nodeId);
      } else {
