...
function getStrGeometryPropertiesHTML(node){
  ...
  if(!rec){
    ...
    left += '<div style="margin-top:9px; padding:7px 9px; font-size:var(--fs-micro); line-height:1.45; color:var(--text-muted); background:var(--bg-secondary); border:1px solid var(--border-color);">'
          + 'Geometri çekirdeği (OpenCascade) <b>uygulamanın içinde</b> — internet gerekmez, '
          + '<b>çevrimdışı çalışır</b>. İlk içe aktarmada bir kez hazırlanır (birkaç saniye), '
          + 'sonrakiler anında. Çok gövdeli dosyalar <b>tek katıya birleştirilir</b>.'
          + '</div>';
