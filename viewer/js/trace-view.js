...
function veTrGeometry(ctx, lanes, view, w, availH) {
  ...
  lanes.forEach(function(lane) {
    ...
    lane._nameW = multi ? Math.min(nameW, VE_TR.NAME_MAX_W) : 0;
    total += multi ? (lane._nameW + VE_TR.NAME_GAP) : VE_TR.TITLE_BAND;
  ...
  lanes.forEach(function(lane) {
    ...
    // yaslı. Tek eksenli şeritte ad bloğu yok, sınır eski TITLE_BAND.
    var leftEdge = multi ? (lane._nameW + VE_TR.NAME_GAP) : VE_TR.TITLE_BAND;
    lane._nameX = multi ? 4 : null;
    var off = 0;
    axes.forEach(function(a) {
...
function veTrDrawAxisNames(ctx, geo, lane, rect) {
  var axes = lane.axes || [];
  if(axes.length < 2 || lane._nameX == null) return;

  var LH = VE_TR.NAME_LINE_H;
  var maxLines = Math.max(1, Math.floor((rect.h - 4) / LH));
  var shown = Math.min(axes.length, maxLines);
  ...
  for(var i = 0; i < shown; i++) {
    ...
    var t = veTrAxisTitle(a) + (a._fits === false ? ' (sığmadı)' : '');
    ctx.fillText(veTrFitTitle(ctx, t, textMax), textX, y);
    y += LH;
  }
...
function veTrBindPopupDismiss(pop) {
  ...
  // setTimeout: pencereyi AÇAN tıklamanın kendisi kapatıcıya düşmesin.
  setTimeout(function() {
