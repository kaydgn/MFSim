 ...
 *
 *   1) OLAĞAN — `veFeadArrangeByCoords` koşuyor (tablo üstte, kanvaslar yan
 *      yana). Bu yol Node'da da ölçülebiliyor.
 *
...
const olc = (page) => page.evaluate(() => {
  ...
  const tb = gorunen.find((a) => a.tip === 'fead-table');
...
function kapilar(r) {
  ...
  // TABLO ÜSTTE — giriş yüzeyi önce okunuyor, blok da dikdörtgen kalıyor.
  expect(r.tablo).toBeTruthy();
  expect(r.tablo.y + r.tablo.h).toBeLessThanOrEqual(r.kanvas[0].y);
...
test('örnek yüklenince: tablo üstte, kanvaslar YAN YANA', async ({ page }) => {
  const hatalar = [];
