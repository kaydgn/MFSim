 ...
 *     _veFEAMeshWithTetgenOrVoxel, _veFEATetMeshNamedSelections).
...
describe('fea-mesh.js — TetGen entegrasyonu', () => {
  test('_veFEAShouldTryTetgen ve _veFEAMeshWithTetgenOrVoxel tanımlı', () => {
    const src = fs.readFileSync(path.join(ROOT, 'js/fea-mesh.js'), 'utf8');
    expect(src).toMatch(/function _veFEAShouldTryTetgen/);
    expect(src).toMatch(/function _veFEAMeshWithTetgenOrVoxel/);
  ...
  test('veFEAMeshFromGeometryAsync TetGen yolunu önce dener', () => {
    const src = fs.readFileSync(path.join(ROOT, 'js/fea-mesh.js'), 'utf8');
    expect(src).toMatch(/_veFEAMeshWithTetgenOrVoxel\(parsed[^,]*,\s*geometry/);
    // Sync sonuç voxelMode ise TetGen tekrar denenir
